...
use std::{fs, io, net::TcpListener, process::Command, time::Instant};
use tokio::signal;
use tokio::sync::oneshot;
use webbrowser;

// State to track start time and port
struct AppState {
    start_time: Instant,
    port: u16,
}
...
fn find_available_port() -> io::Result<u16> {
    ...
    let end_port = 65535;
    (start_port..end_port)
        .find_map(|port| TcpListener::bind(("127.0.0.1", port)).map(|_| port).ok())
        .ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "No available port found"))
}
...
function displayInfo() {
  ...
  fetch("/server-info")
    ...
    .then((info) => {
      ...
      const infoText = `**xTerm jsh v1.2** - A JS development environment for students in your browser.
...
function parseMarkdown(mdText) {
  return mdText
    ...
    let mut html_content = include_str!("../static/index.html").to_string();

    // Append JavaScript code to resize the window
    let resize_script = r#"
        <script>
            // Resize the window to the specified dimensions
            function resizeWindow() {
                window.resizeTo(800, 535);
            }

            // Call the resizeWindow function when the window is resized or maximized
            window.addEventListener('resize', resizeWindow);
            window.addEventListener('maximize', resizeWindow);
    ...
    // Append resize script to the end of the HTML content
    html_content.push_str(resize_script);
...
async fn close_server() -> impl Responder {
    println!("Server is shutting down by request...");
    ...
    match Command::new("cargo").arg("clean").output() {
        Ok(output) if output.status.success() => {
            println!("Dependencies cleaned successfully.");
        }
        _ => {
            eprintln!("Failed to clean dependencies.");
        }
    }
...
async fn main() -> io::Result<()> {
    ...
    let server = HttpServer::new(move || {
        App::new()
            ...
            .route("/close", web::post().to(close_server))
    ...
    tokio::spawn(async move {
        ...
        tx.send(()).expect("Failed to send shutdown signal"); // Send a shutdown signal
        let _ = server_handle.stop(true);
    });

    if webbrowser::open(&format!("http://127.0.0.1:{}", port)).is_err() {
    ...
    tokio::select! {
        _ = server => {},
        _ = rx => {
            println!("Shutdown signal received from the close endpoint.");
        },
    }
